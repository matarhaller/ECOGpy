...
def HG_regression_allelecs_SGE(DATASET):
    ...
    *** added surrogate 12/12/14
    ...
    reg_dict = HG_regression_allelecs(subj, task, surrogate = surrogate, id_num = ID, static = static, folder = folder, circshift = circshift)
    
    if static:
        saveDir = os.path.join(SJdir, 'PCA', 'Stats', 'Regression', 'static')
    else:
        saveDir = os.path.join(SJdir, 'PCA', 'Stats', 'Regression', 'unsmoothed', folder, 'no_rejection')
    ...
    if surrogate:
        ...
        filename = os.path.join(saveDir, '_'.join([subj, task, 'circ']))
        if not(os.path.exists(saveDir)):
            os.mkdir(saveDir)
    else:
...
def HG_regression_allelecs(subj, task, surrogate = False, id_num = 99, static = False, folder = 'tmp', circshift = False):
    '''
    ...
    if static:
        ...
        if surrogate:
            filename = os.path.join(SJdir, 'PCA', 'Stats', 'outliers', 'for_Regression', 'unsmoothed', folder, 'no_rejection', 'surr_'+ str(id_num), '_'.join([subj, task, 'surr'])) 
            print ('loading surrogate data from\n%s' %(filename))
            sys.stdout.flush()
        elif circshift:
            filename = os.path.join(SJdir, 'PCA', 'Stats', 'outliers', 'for_Regression', 'unsmoothed', folder, 'no_rejection', 'circshift', '_'.join([subj, task, 'circ'])) 
            print ('loading circshift data from\n%s' %(filename))
            sys.stdout.flush()
        else:
            filename = os.path.join(SJdir, 'PCA', 'Stats', 'outliers', 'for_Regression', 'unsmoothed', folder, 'no_rejection', '_'.join([subj, task])) 
