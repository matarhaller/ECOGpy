...
saveDir = '/home/knight/matar/MATLAB/DATA/Avgusta/PCA/plots_dyn/'
...
for (i in 1:nrow(d) ) {
  ...
  #result = cutreeDynamic(clusters, method = 'hybrid', distM = as.matrix(distance))
  #nclust = max(result)
  
  #plot(silhouette(cutree(clusters,2),distance))
  ...
  b = rect.hclust(clusters, k = nclust, cluster = result, border="blue") 
...
filename = paste(saveDir, 'groupidx_dyn.csv',sep = "")
