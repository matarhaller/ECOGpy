...
def shadeplots_elecs_stats():
    """ 
    ***uses circshifted HG data matrix***
    ...
    *** runs on unsmoothed data (12/11/14)***
    ...
    saveDir_csv = os.path.join(SJdir, 'PCA', 'ShadePlots_hclust', 'elecs', 'significance_windows', 'unsmoothed', 'csv_files', 'orig', 'circshift')
    saveDir_data= os.path.join(SJdir, 'PCA', 'ShadePlots_hclust', 'elecs', 'significance_windows', 'unsmoothed', 'data', 'circshift')
    ...
    for s_t in df.groupby(['subj','task']):
        ...
        subj, task = s_t[0]
        #load data
        filename = os.path.join(SJdir, 'Subjs', subj, task, 'HG_elecMTX_percent_unsmoothed_surr.mat') #created from circshifted gdat
        ...
        filename = os.path.join(saveDir_data, ''.join([subj, '_', task, '_circshift.p']))
        ...
        for k in data_dict.keys():
            ...
            filename = os.path.join(saveDir_csv, '_'.join([subj, task, k]) + '_circshift.csv')
            data.to_csv(filename, index = False)

    #save dataframe with dropped trials
    filename = os.path.join(SJdir,'PCA', 'Stats', 'single_electrode_windows_withdesignation_EDITED_dropped_unsmoothed_circshift.csv')
    df.to_csv(filename)
