...
saveDir = '/home/knight/matar/MATLAB/DATA/Avgusta/PCA/plots_mclust'
...
for (i in 1:nrow(d) ) {
  ...
  d_clust <- Mclust(as.matrix(fit$loadings), G=1:15)
  nclust = d_clust$G

  
  distance = dist(fit$loadings, method = 'correlation') #calculate distance between weights
  clusters<-hclust(distance, method = 'complete') #calculate hierarchical clusters
  #groups <- cutree(clusters, k=nclust)
  groups = d_clust$classification
  
  #add groups to dataframe
  df = rbind(df, data.frame(subj, task, active_elecs, groups))
  
  #plot tree
  a = plot(clusters,labels = as.character(active_elecs), main=paste(subj,task, sep = ' '))
  if (nclust==1) nclust<-2 #so can plot 1 big box if only have 1 cluster
  b = rect.hclust(clusters, k = nclust, cluster = groups, border="magenta") 
  filename = paste(saveDir, subj, "_", task, '_hclust_mclust.png',sep = "")
  dev.copy(png,filename)
  dev.off()
...
wwfilename = paste(saveDir, 'groupidx_mclust.csv',sep = "")
